...
def first_task():
    ...
    # inversion using zip
    plur2sing = {a: b for a, b in zip(sing2plur.values(),sing2plur.keys())}  # vase reseni
    return plur2sing == {'geese': 'goose', 'men': 'man', 'children': 'child'}


def second_task():
    word2vect = {'king': [3, 1], 'queen': [6, 3], 'uncle': [4, 3], 'aunt': [8, 9]}
    vect2word = {tuple(a): b for a, b in zip(word2vect.values(), word2vect.keys())}  # vase reseni
...
class MyList(list):
    def __hash__(self):
        return hash(tuple(self))  # nahradte za vase reseni
